...
class Store implements StoreInterface
{
    ...
    public function get($key)
    {
        ...
        if ( Str::contains($key, ':timer') ) {
            ...
            $datetime = $expiration ? (new UTCDateTime($expiration))->toDateTime()->format('Y-m-d H:i:s') : false;
            $now = date('Y-m-d H:i:s');

            return ( $datetime && $datetime >= $now && $cacheData ) ? unserialize($cacheData['value']) : null;
        }
